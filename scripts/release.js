...
function isDevelopBranch() {
  return /feature\//.test(currentBranchName);
}

// function isReleaseBranch() {
//   return /release\//.test(currentBranchName)
// }

// function isMasterBranch() {
//   return /master/.test(currentBranchName)
// }

async function analyzeCurrentBranch() {
  try {
    let { stdout } = await run("git", ["branch"], { stdio: "pipe" });
    ...
    throw Error("no parse current branch? ");
  } catch (error) {
    console.log(error);
    process.exit(1);
  }
...
async function main() {
  ...
  if (!targetVersion) {
    await analyzeCurrentBranch();
    targetVersion = await getTargetVersion();
...
function updateDeps(pkg, depType, version) {
  ...
  Object.keys(deps).forEach((dep) => {
    if (dep.startsWith("@wosai/pode") && packages.includes(dep.replace(/^@wosai\/pode-/, ""))) {
